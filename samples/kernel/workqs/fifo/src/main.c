...
WORKQ_FIFO_THREAD_DEFINE(thread1, my_workq, 1024, PRIORITY);
WORKQ_FIFO_THREAD_DEFINE(thread2, my_workq, 1024, PRIORITY);
...
void sample_multiple_workers(void)
{
	...
	for (size_t i = 0; i < 5; i++) {
		...
		workq_fifo_delayed_submit(&my_workq, &c->item, K_MSEC(delay));
	}
	...
	workq_fifo_drain(&my_workq, K_FOREVER);
	LOG_INF("All work items have been processed");
}

static void sync_fn(struct work_base *item)
{
	struct sync *s = CONTAINER_OF(item, struct sync, item.base);
