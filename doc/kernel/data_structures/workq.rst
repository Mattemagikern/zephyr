...
* **Unified work items** – A single :c:struct:`work_item` type is used for both immediate and
  delayed work. Because the queue owns the lifecycle, membership is tracked by the item's list
  linkage, so submitting, cancelling, and rescheduling are constant-time regardless of queue depth.
