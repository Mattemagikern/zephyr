...
static ALWAYS_INLINE void z_spinlock_validate_pre(struct k_spinlock *l)
{
	...
	ZASSERT(z_spin_lock_valid(l), "Invalid spinlock %p", l);
#ifdef CONFIG_KERNEL_COHERENCE
...
#ifdef CONFIG_SPIN_VALIDATE
	ZASSERT_MODULE(KERNEL);
	ZASSERT(z_spin_unlock_valid(l), "Not my spinlock %p", l);

#if defined(CONFIG_SPIN_LOCK_TIME_LIMIT) && (CONFIG_SPIN_LOCK_TIME_LIMIT != 0)
	uint32_t delta = sys_clock_cycle_get_32() - l->lock_time;

	ZASSERT(delta < CONFIG_SPIN_LOCK_TIME_LIMIT,
...
{
	ZASSERT_MODULE(KERNEL);
	ZASSERT(k->key, "K_SPINLOCK exited with goto, break or return, "
