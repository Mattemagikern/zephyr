...
/**
 * @brief wait for a work item to finish
 * @param wq Work queue to flush
 * @param item Work item to flush
 * @param timeout Maximum time to wait for work to finish
 * @retval 0 if work was finished in time, -EAGAIN if timed out
 */
 ...
 * @param timeout Maximum time to wait for work to be available
 * @retval 0 if work was drained, -EAGAIN if timed out
 */
int workq_drain(struct workq *wq, k_timeout_t timeout);

/**
 * @brief workq_work_status - Get the status of a work item
 * @param wq Work queue to check
 * @param item Work item to check
 * @return Status of the work item
 * @retval WORKQ_STATUS_IDLE if the work item is not in the queue
 * @retval WORKQ_STATUS_RUNNING if the work item is currently running
 * @retval WORKQ_STATUS_PENDING if the work item is in the queue
 */
...
{								\
	...
	.flush = SYS_SLIST_STATIC_INIT(&obj.flush),		\
