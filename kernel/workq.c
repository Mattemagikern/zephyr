...
LOG_MODULE_REGISTER(workq, LOG_LEVEL_INF);
...
/* should only be called with lock held */
static inline void release_barrier(struct workq *wq, uintptr_t work)
{
	sys_snode_t *safe;
	sys_snode_t *node;
	...
	SYS_SLIST_FOR_EACH_NODE_SAFE(&wq->flush, node, safe) {
		barrier = CONTAINER_OF(node, struct barrier, node);
		if (barrier->work == work) {
			sys_slist_remove(&wq->flush, prev, node);
			k_sem_give(&barrier->sem);
			break;
		}
		prev = node;
	}
...
static inline int cancel(struct workq *wq, struct work_item *item)
{
	...
	if (!pending(wq, item)) {
		release_barrier(wq, (uintptr_t)item);
	}
...
static void schedule_cb(struct _timeout *t)
{
	...
	K_SPINLOCK(&wq->lock) {
		node = sys_slist_get(&wq->delayed);
		if (node == NULL) {
			LOG_ERR("No delayed work item found, implementation error");
			K_SPINLOCK_BREAK;
		}
		item = CONTAINER_OF(node, struct work_item, node);
...
int workq_run(struct workq *wq, k_timeout_t timeout)
{
	...
	fn(work); /* work should be able to be freed here */

	K_SPINLOCK(&wq->lock) {
		sys_slist_find_and_remove(&wq->active, &active.node);
		if (!pending(wq, work)) {
			release_barrier(wq, active.work);
		}
		if (workq_idle(wq)) {
			z_sched_wake_all(&wq->drain, 0, NULL);
		}
...
int workq_sync(struct workq *wq, struct work_item *item, k_timeout_t timeout)
{
	struct barrier barrier;

	k_sem_init(&barrier.sem, 0, 1);
	barrier.work = (uintptr_t)item;

	K_SPINLOCK(&wq->lock) {
		sys_slist_append(&wq->flush, &barrier.node);
	}

	if (k_sem_take(&barrier.sem, timeout)) {
		K_SPINLOCK(&wq->lock) {
			release_barrier(wq, (uintptr_t)item);
		}
		return -EAGAIN;
...
enum work_status workq_work_status(struct workq *wq, struct work_item *item)
{
	enum work_status rc;

	K_SPINLOCK(&wq->lock) {
		if (active(wq, item)) {
			rc = WORK_STATUS_RUNNING;
		} else if (pending(wq, item)) {
			rc = WORK_STATUS_PENDING;
		} else {
			rc = WORK_STATUS_IDLE;
		}
	}
