...
{
	return sys_dlist_is_empty(&wq->pending) && sys_dlist_is_empty(&wq->delayed) &&
		sys_slist_is_empty(&wq->active);
}

/* should only be called with lock held */
static inline bool queued(struct work_item *item)
{
	return sys_dnode_is_linked(&item->node);
}
...
{
	sys_dnode_t *node = sys_dlist_get(&wq->pending);
...
static void schedule_next_timeout(struct workq *wq)
{
	...
	if (sys_dlist_is_empty(&wq->delayed)) {
		return;
	}

	item = CONTAINER_OF(sys_dlist_peek_head(&wq->delayed), struct work_item, node);
...
static inline int cancel(struct workq *wq, struct work_item *item)
{
	...
	if (sys_dlist_is_head(&wq->delayed, &item->node)) {
...
static inline void delayed_submit(struct workq *wq, struct work_item *item, k_timepoint_t exec_time)
{
	...
	SYS_DLIST_FOR_EACH_NODE(&wq->delayed, node) {
		next = CONTAINER_OF(node, struct work_item, node);
		if (sys_timepoint_cmp(item->exec_time, next->exec_time) < 0) {
			sys_dlist_insert(node, &item->node);
			break;
		}
	}

	if (node == NULL) {
		sys_dlist_append(&wq->delayed, &item->node);
	}

	if (sys_dlist_peek_head(&wq->delayed) == &item->node) {
		(void)z_try_abort_timeout(&wq->timeout);
		schedule_next_timeout(wq);
	}
}
...
static void schedule_cb(struct _timeout *t)
{
	...
	K_SPINLOCK(&wq->lock) {
		while (!sys_dlist_is_empty(&wq->delayed)) {
			item = CONTAINER_OF(sys_dlist_peek_head(&wq->delayed), struct work_item, node);
			if (!sys_timepoint_expired(item->exec_time)) {
				break;
			}
			sys_dlist_get(&wq->delayed);
			sys_dlist_append(&wq->pending, &item->node);
			awake(wq);
		}
		schedule_next_timeout(wq);
	}
...
void workq_init(struct workq *wq)
{
	...
	sys_dlist_init(&wq->pending);
	sys_dlist_init(&wq->delayed);
