...
WORKQ_FIFO_DEFINE(test_define_workq);
WORKQ_FIFO_THREAD_DEFINE(test_define_wqt, test_define_workq, WORKQ_TEST_STACK_SIZE, 0);
...
ZTEST(basic, test_macro_defined_workq)
{
	...
	zassert_true(workq_fifo_run(&test_define_workq, K_NO_WAIT) == -EAGAIN,
			"workq have no work, should return -EAGAIN");

	zassert_true(workq_fifo_thread_start(&test_define_wqt) == -EALREADY,
			"workq thread already started, should return -EALREADY");
	zassert_true(workq_fifo_thread_stop(&test_define_wqt, K_MSEC(10)) == 0,
...
ZTEST(basic, test_setup)
{
	...
	LOG_INF("----------------------------");
	LOG_INF("sizeof work %zu", sizeof(struct workq_fifo_work));
	LOG_INF("sizeof q %zu", sizeof(struct workq_fifo));
	LOG_INF("sizeof wqt %zu", sizeof(struct workq_thread));
	LOG_INF("----------------------------");
	LOG_INF("sizeof workq.flags %zu", sizeof(q.engine.flags));
	LOG_INF("sizeof workq.lock %zu", sizeof(q.engine.lock));
	LOG_INF("sizeof workq.timeout %zu", sizeof(q.engine.timeout));
	LOG_INF("sizeof workq.active %zu", sizeof(q.engine.active));
	LOG_INF("sizeof workq.pending %zu", sizeof(q.engine.pending));
	LOG_INF("sizeof workq.delayed %zu", sizeof(q.engine.delayed));
	LOG_INF("sizeof workq.idle %zu", sizeof(q.engine.idle));
	LOG_INF("sizeof workq.drain %zu", sizeof(q.engine.drain));
	LOG_INF("----------------------------");
	...
	workq_fifo_init(&q);
	zassert_true(workq_fifo_run(&q, K_NO_WAIT) == -EAGAIN,
			"workq have no work, should return -EAGAIN");
}
...
ZTEST(basic, test_submit)
{
	...
	for (size_t i = 0; i < 3; i++) {
		zassert_ok(workq_fifo_run(&q, K_NO_WAIT), "workq_fifo_run failed");
...
ZTEST(basic, test_submit_delayed)
{
	...
	for (size_t i = 0; i < 3; i++) {
		...
		workq_fifo_work_init(&c->item, work_fn);
		c->sem = &sem;
		zassert_true(workq_fifo_delayed_submit(&q, &c->item, K_MSEC(10 + i*10)) == 0,
				"workq_fifo_submit failed");
	}

	for (size_t i = 0; i < 3; i++) {
		zassert_ok(workq_fifo_run(&q, K_MSEC(50)), "workq_fifo_run failed");
		zassert_ok(k_sem_take(&sem, K_NO_WAIT), "work_fn not called");
	}
}
...
ZTEST(basic, test_cancel)
{
	...
	zassert_true(workq_fifo_delayed_submit(&q, &c->item, K_MSEC(50)) == 0,
			"workq_fifo_submit failed");
	zassert_ok(workq_fifo_cancel(&q, &c->item), "workq_fifo_cancel failed");
	zassert_true(workq_fifo_run(&q, K_MSEC(100)) == -EAGAIN, "workq_fifo_run should timeout");
	zassert_not_ok(k_sem_take(&sem, K_NO_WAIT), "work_fn called?");
}

static void work_fn_nonfree(struct work_base *item)
{
...
static void work_fn_block(struct work_base *item)
{
...
ZTEST(basic, test_drain)
{
	...
	struct workq_fifo q;
	struct workq_fifo_work item;
	struct workq_fifo_work item2;

	workq_fifo_init(&q);
	workq_fifo_work_init(&item, work_fn_nonfree);
	workq_fifo_work_init(&item2, work_fn_nonfree);
	workq_fifo_thread_init(&wqt, &q, stack, K_THREAD_STACK_SIZEOF(stack), NULL);

	zassert_ok(workq_fifo_thread_start(&wqt), "workq_fifo_thread_start failed");
	zassert_ok(workq_fifo_delayed_submit(&q, &item, K_MSEC(500)), "workq_fifo_submit failed");
	zassert_true(-EAGAIN == workq_fifo_drain(&q, K_MSEC(200)), "workq_fifo_drain failed");
	zassert_ok(workq_fifo_drain(&q, K_MSEC(350)), "workq_fifo_drain failed");

	zassert_ok(workq_fifo_thread_stop(&wqt, K_MSEC(100)), "Failed to stop workq thread");
}
...
ZTEST(basic, test_reschedule)
{
	...
	zassert_ok(workq_fifo_thread_start(&wqt), "workq_fifo_thread_start failed");
	zassert_ok(workq_fifo_delayed_submit(&q, &item, K_MSEC(500)), "workq_fifo_submit failed");
	zassert_ok(workq_fifo_reschedule(&q, &item, K_MSEC(100)), "workq_fifo_reschedule failed");
	zassert_ok(workq_fifo_drain(&q, K_MSEC(200)), "workq_fifo_drain failed");
...
ZTEST(basic, test_open_close)
{
	...
	workq_fifo_close(&q);
	zassert_true(-EAGAIN == workq_fifo_submit(&q, &c->item),
			"workq_fifo_submit should fail when closed");

	workq_fifo_open(&q);
	zassert_ok(workq_fifo_submit(&q, &c->item), "workq_fifo_open failed");
	zassert_ok(workq_fifo_run(&q, K_MSEC(100)),
			"workq_fifo_run should have successfully run the work");
	zassert_ok(k_sem_take(&sem, K_NO_WAIT), "work_fn not called");
}
...
ZTEST(basic, test_freeze_thaw)
{
	...
	workq_fifo_init(&q);
	zassert_ok(workq_fifo_delayed_submit(&q, &c->item, K_MSEC(50)), "workq_fifo_submit failed");
	workq_fifo_freeze(&q);
	...
	k_sem_init(&sem2, 0, 1);
	c2->sem = &sem2;
	workq_fifo_work_init(&c2->item, work_fn);
	zassert_ok(workq_fifo_submit(&q, &c2->item), "workq_fifo_submit failed");
	zassert_ok(workq_fifo_run(&q, K_MSEC(100)),
			"workq_fifo_run should have successfully run the work");
	...
	k_msleep(100);
	zassert_true(workq_fifo_submit(&q, &c->item) == -EALREADY,
			"work should already be submitted");
	workq_fifo_thaw(&q);
	zassert_ok(workq_fifo_run(&q, K_MSEC(10)),
			"workq_fifo_run should have successfully run the work");
	zassert_ok(k_sem_take(&sem, K_NO_WAIT), "work_fn not called");
}
...
ZTEST(basic, test_submit_already)
{
	...
	zassert_ok(workq_fifo_submit(&q, &item), "first submit should succeed");
	zassert_equal(-EALREADY, workq_fifo_submit(&q, &item),
...
ZTEST(basic, test_cancel_enoent)
{
	...
	zassert_equal(-ENOENT, workq_fifo_cancel(&q, &item),
...
ZTEST(basic, test_cancel_busy)
{
	...
	workq_fifo_init(&q);
	workq_fifo_work_init(&item, work_fn_block);
	workq_fifo_thread_init(&wqt, &q, busy_stack, K_THREAD_STACK_SIZEOF(busy_stack), NULL);

	zassert_ok(workq_fifo_thread_start(&wqt), "workq_fifo_thread_start failed");
	zassert_ok(workq_fifo_submit(&q, &item), "workq_fifo_submit failed");
	zassert_ok(k_sem_take(&started_sem, K_MSEC(100)), "work did not start");

	zassert_equal(-EBUSY, workq_fifo_cancel(&q, &item),
...
ZTEST(basic, test_closed_delayed_submit)
{
	...
	zassert_equal(-EAGAIN, workq_fifo_delayed_submit(&q, &item, K_MSEC(10)),
			"workq_fifo_delayed_submit on a closed queue should return -EAGAIN");
	zassert_equal(-EAGAIN, workq_fifo_reschedule(&q, &item, K_MSEC(10)),
			"workq_fifo_reschedule on a closed queue should return -EAGAIN");
}
...
ZTEST(basic, test_thread_start_uninitialized)
{
	...
	zassert_equal(-ENODEV, workq_fifo_thread_start(&wqt),
...
ZTEST(basic, test_stop_idle_worker)
{
	...
	zassert_ok(workq_fifo_thread_stop(&wqt, K_MSEC(100)),
...
ZTEST(basic, test_thaw_not_frozen)
{
	...
	zassert_ok(workq_fifo_submit(&q, &item), "queue should stay usable after thaw");
	zassert_ok(workq_fifo_run(&q, K_NO_WAIT), "workq_fifo_run failed");
}
